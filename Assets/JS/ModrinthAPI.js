            ...
            .then((result) => {
                ...
                for (let version in json) {
                    ...
                    if (json[version].status === "listed") {
                        versionsGrid += `<a href="https://modrinth.com/plugin/${asset}/version/${json[version].version_number}" class="card-link" id="${json[version].version_number}"><h3>${json[version].name}</h3><p>For Minecraft ${json[version].game_versions[0]} - ${json[version].game_versions[json[version].game_versions.length-1]}</p></a>`;
                    }
                }
